...
import org.openmoney.omlets.mobile.client.accounts.AccountDetailsPage;
import org.openmoney.omlets.mobile.client.accounts.AccountsPage;
import org.openmoney.omlets.mobile.client.configuration.ConfigurationPage;
...
import org.openmoney.omlets.mobile.client.payments.MakePaymentPage;
import org.openmoney.omlets.mobile.client.payments.PaymentDetailsPage;
import org.openmoney.omlets.mobile.client.payments.PaymentPreviewPage;
import org.openmoney.omlets.mobile.client.payments.PaymentsPage;
...
import org.openmoney.omlets.mobile.client.users.UserSelectPage;
...
public class Navigation {
    ...
    private PageAnchor getInitialPageAnchor() {
        return getInitialPageAnchor(OmletsMobile.get().getAppState());
    }
    ...
    private void handle(String token) {               
        ...
        if(StringHelper.isEmpty(token) || exitOnHistoryChange) {
            ...
            OmletsMobile.get().getPhoneGap().exitApp();
            return;
        }
        ...
        Page currentPage = OmletsMobile.get().getMainLayout().getCurrentPage();
        ...
        if(isPageAllowed(page)) {
            ...
            // Go to allowed page
            AppState status = OmletsMobile.get().getAppState();   
    ...
    private boolean isPageAllowed(Page page) {
        AppState appStatus = OmletsMobile.get().getAppState();                  
...
//                break;
    ...
    private void go(Page page) {
        ...
        OmletsMobile.get().getMainLayout().show(page);
    }
